...
const SelectMultipleSearch: React.FC<SelectMultipleProps> = ({
  label,
  placeholder = label,
  error,
  options,
  ...
  const [selectedOptions, setSelectedOptions] = useState<SelectOption[]>(options.filter((option) => value.includes(option.key)));
          ...
          <ul className="max-h-48 overflow-y-auto">
            {filteredOptions.length > 0 ? (
              filteredOptions.map((option) => (
                <li key={option.key} className="cursor-pointer px-2.5 py-2.5 hover:bg-background-main-card flex items-center">
